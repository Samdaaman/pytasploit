...
def process_requests_forever():
    while True:
        message = config.messages_received.get()  # type: Any
        ...
        if handle_func is not None and inspect.isfunction(handle_func):
            ...
            print(f'Could not find handle_func for {message.encode()}')
...
def _handle_SelfDestructRequest(request: SelfDestructRequest):
...
def _handle_GoStealthyRequest(request: GoStealthyRequest):
